...
| Grok | `Grok/COPY-TO-PROVIDER-HOME/config.toml` | `%USERPROFILE%\.grok\config.toml` | Copy if missing; otherwise keep live MCP and ensure `mcp-search` is disabled. Budget: 7 configured / 6 while a plugin server loads. |

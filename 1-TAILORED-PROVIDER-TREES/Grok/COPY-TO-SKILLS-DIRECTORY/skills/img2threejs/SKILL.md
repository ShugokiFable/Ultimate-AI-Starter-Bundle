...
This is the bundle-managed v1.5.1 copy. Do not clone, symlink, or update it into a provider's live
skills directory: `_CANONICAL-SKILLS` is the source of truth and the bundle fans the same reviewed
bytes out to every provider.

On Windows, run every `python3 forge/...` command below through the included resolver so UTF-8 and
the actual installed Python are deterministic:

```powershell
powershell -NoProfile -ExecutionPolicy Bypass -File <skill-base-dir>\scripts\uabs-python.ps1 forge/next.py --state .img2threejs/state.json
```

Pass the script path first and all of its normal arguments after it. On macOS/Linux, use the
upstream `python3` commands as written.

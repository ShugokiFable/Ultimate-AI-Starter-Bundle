...
| `oauth_anthropic_creds` | Partner provides `client_id`/`client_secret` to Anthropic; user-consent-gated. Contact Anthropic's MCP review team (address in the MCP connector docs). |
| `custom_connection` | User supplies URL/creds at connect time (Snowflake-style). Contact Anthropic's MCP review team (address in the MCP connector docs). |
| `none` | Authless. |

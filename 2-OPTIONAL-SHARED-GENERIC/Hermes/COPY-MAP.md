# Hermes shared V5 copy map

Copy all skills into `%HERMES_HOME%\skills`. Copy the workspace instruction file into any user-selected workspace root. No workspace path is assumed. The error registry is embedded in `skyrim-memory\references`.

The preflight guard is at `%HERMES_HOME%\skills\skyrim-versioned-workspace\scripts\guard.py`. Run it before any write or shell command.
## V5 extras

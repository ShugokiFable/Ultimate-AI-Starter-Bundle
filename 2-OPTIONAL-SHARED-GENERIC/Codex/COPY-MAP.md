# Codex shared V5 copy map

Copy all skills into `%CODEX_HOME%\skills`. Copy the workspace instruction file into any user-selected workspace root. No workspace path is assumed. The error registry is embedded in `skyrim-memory\references`.
